...
/* $Id: log_lib.h,v 1.39 2009/07/07 14:28:25 anthony Exp $ */
 ...
 *pTH;
  ...
  typedef ht *pTH;
  typedef htitem *pElemTH;

#define createTH addht
#define destroyTH delht
#define searchTH gethtitem
#define addTH addhtitem
#define addExistTH sethtitem
#define deleteTH delhtitem
...
#define VECT_S(expr)        (*(long *)&CADR(CAR(expr)))
#define REP_NB(expr)        (*(long *)&CADR(CDAR(expr))) 
#define ATTRIB(expr)        CDAR(expr)
#define VALUE_ATTRIB(expr)  REP_NB(expr)
...
/* Prototypes from log_bdd0.c */
  ...
  extern             int bddSystemAbandoned __P((void));
  extern            void setBddCeiling __P((int limit));
  extern             int getBddCeiling __P((void));
  extern            void unsetBddCeiling __P((void));
  ...
  extern            void displayBddNameLog __P((int lib, int loglevel, short level, pNode pt, char **TabName));
...
/* Prototypes from log_bdd1.c */
  ...
  extern           pNode searchOutputCct_no_NA __P((pCircuit pC, char *name));
  extern            void addOutputCct __P((pCircuit pC, char *name, pNode pt));
  extern            void addOutputCct_no_NA __P((pCircuit pC, char *name, pNode pt));
  extern          char * searchIndexCct __P((pCircuit pC, short index));
  extern           short searchInputCct __P((pCircuit pC, char *name));
  extern           short searchInputCct_no_NA __P((pCircuit pC, char *name));
  extern           short addInputCct __P((pCircuit pC, char *name));
  extern           short addInputCct_no_NA __P((pCircuit pC, char *name));
  extern            void delInputCct __P((pCircuit pC, char *name));
  extern            void displayCctLog __P((int lib, int loglevel, pCircuit pC, int mode));
...
/* Prototypes from log_prefbib.c */
  ...
  extern    chain_list * createAtom_no_NA __P((char *name));
  ...
  extern            void displayExprLog __P((int lib, int loglevel, chain_list *expr));
  extern            void displayInfExprLog __P((int lib, int loglevel, chain_list *expr));
  ...
  extern            void fullSupportChain_listExprInt __P((chain_list *expr, chain_list **ppCL));
  extern    chain_list * fullSupportChain_listExpr __P((chain_list *expr));
  ...
  extern            void replaceAtomByExpr __P((chain_list *expr, char *name, chain_list *newexpr));
...
/* Prototypes from log_thashbdd.c */
  ...
  extern            long hashBdd __P((int index, pNode high, pNode low));
  extern            long newKeyBdd __P((int index, pNode high, pNode low));
...
/* Prototypes from log_vectabl.c */
  extern    chain_list * concatAbl __P((chain_list *expr1,chain_list *expr2));
  extern    chain_list * replicateAbl __P((chain_list *expr ,int  n));
  extern    chain_list * getAblAtPos __P((chain_list *expr ,int  pos));
  extern             int getVectAblRange __P((chain_list *expr ,int *left,int *right));
  extern          char * getVectAblVarName __P((chain_list *expr));
  extern             int verifyVectAbl __P((chain_list *expr));
  extern             int getAtomSize __P((chain_list *expr));
  extern    chain_list * getAblAtIndex __P((chain_list *expr,int left,int right,int index));
  extern            void invertVectAbl __P((chain_list *expr));
  extern            void invertAtom __P((chain_list *expr));
  extern            void initBitStrTable __P((void));
  extern             int isBitStr __P((char *bitstr));
  extern          char * genOneBitStr __P((int n));
  extern          char * genZeroBitStr __P((int n));
  extern          char * genHZBitStr __P((int n));
  extern            void fprintHZBitStr __P((FILE *file,char *bitstr));
  extern          char * notBitStr __P((char *bitstr));
  extern    chain_list * createBitStr __P((char *bitstr));
  extern          char * genUBitStr __P((int n));
  extern          char * getBitStr __P((char *str));
  extern             int changeAblAtRange __P((chain_list *expr,int left,int right, chain_list *added,int mode));
  extern          char * renameVectAtom __P((char *oldname,char *newname,int shift));
  extern    chain_list * createVectAtom __P((char *name,int left,int right));
  extern          char * createVectAtomName __P((char *name,int left,int right));
  extern            void getVectAtomRange __P((chain_list *expr,char **name,int *left, int*right));
  extern            void repToCatVectAbl __P((chain_list *expr));
  extern          char * makeBitStr __P((char *bistr));
  extern FILE *LOG_DEBUG_FILE;
  extern    chain_list * supportChain_listBddExpr(pCircuit circuit, pNode bdd);
